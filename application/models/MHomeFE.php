...
class MHomeFE extends CI_Model
{
	...
	public function bestSeller()
	{
		...
		$sql = $this->db->query("SELECT tb_products.*, tb_ships_list.* FROM tb_products INNER JOIN tb_ships_list ON tb_products.id_ship_list=tb_ships_list.id_ship_list WHERE tb_products.id_status=2 LIMIT 12;");
		return $sql->result();
	}
	...
	public function allProducts()
	{
		...
		$this->db->from('tb_products');
		$this->db->join('tb_ships_list', 'tb_products.id_ship_list = tb_ships_list.id_ship_list');	
		$this->db->where('tb_products.id_status', 2);	
		return $this->db->get()->result();
	}
	...
	public function getDetailsShip($slug)
	{
